...
//	@Override
	...
	public TWEANNGenotype(int numIn, int numOut, int archetypeIndex) {
		this(numIn, numOut, true, ActivationFunctions.FTYPE_TANH, 1, archetypeIndex);
	}
...
//	}
	...
	public void weightMutation() {
		perturbLink(randomAlterableLink(), RandomNumbers.randomGenerator.nextGaussian());
	}
	 ...
	 */
	public void allWeightMutation(double rate) {
		for (LinkGene l : links) {
			if (!l.isFrozen() && RandomNumbers.randomGenerator.nextDouble() < rate) {
				perturbLink(l, RandomNumbers.randomGenerator.nextGaussian());
			}
		}
	}
...
//	@Override
	...
	private long getRandomLinkSourceNodeInnovationNumber() {
		...
		return nodes.get(RandomNumbers.randomGenerator.nextInt(nodes.size() )).innovation;
	}
	...
	private long getRandomAlterableConnectedNodeInnovationNumber(long source, boolean includeInputs) {
		...
		for (LinkGene l : links) {
			...
			if (( sourceIndex < potentialTargetIndex) // recurrent links allowed?
		...
		for (int i = 0; i < numOut; i++) {
			...
			if (sourceIndex < potentialTargetIndex) {
...
//			}
	...
	private void spliceMutation(int ftype) {
		...
		double weight1 = RandomNumbers.fullSmallRand();
		double weight2 = RandomNumbers.fullSmallRand();
...
//	@Override
	 ...
	 */
//	@Override
	public TWEANNGenotype newInstance() {
...
//		} else {
			result = new TWEANNGenotype(4, 3, this.archetypeIndex);
//		}
