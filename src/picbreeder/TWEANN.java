...
public class TWEANN {
	...
	public static boolean finalPassOnOutputActivation = false; //Parameters.parameters.booleanParameter("finalPassOnOutputActivation");
...
//				|| fullMMRate > 0);
	...
	public TWEANN(int numIn, int numOut, boolean featureSelective, int ftype, int numModes, int archetypeIndex) {
		NETWORK_VIEW_DIM = 500;
		this.archetypeIndex = archetypeIndex;
		this.numIn = numIn;
		this.moduleUsage = new int[numModes];
		this.preferenceFatigue = new double[numModes];

		int startingPrefModes = 1; //Parameters.parameters.integerParameter("startingModes");
		...
		int linksPer = 1; //Parameters.parameters.integerParameter("fsLinksPerOut");
...
//		}
		...
		// determine winner
		chosenModule = StatisticsUtilities.argmax(preferences);
...
//	}
	...
	private void checkNode(Graphics2D g, Node display)	 {
		double activation = display.activation;
		if(display.frozen) { // TODO: Is this case even reachable?
		...
		} else { // TODO: Just move this to where the node is drawn in the first place?
